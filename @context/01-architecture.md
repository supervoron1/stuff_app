...
| Файл | Назначение |
|---|---|
| `components/inventory.tsx` | Главный экран: списки, фильтры (скрытие категорий), поиск, модалки |
...
- Подписи статусов едины для UI и аудита: «Есть»/«Мало»/«Нет» — единый источник `STATUS_LABELS` в `lib/constants.ts` (используется в `components/stock-indicator.tsx`, фильтрах, `lib/actions.ts` и `app/api/sync/route.ts`).
