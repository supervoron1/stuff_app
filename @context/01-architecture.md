...
2. **Перетаскивание товара** (drag-n-drop за ручку ⠿ + название):
...
- **Перетаскивание** доступно только в режиме «Все» и без поиска; сенсоры: MouseSensor (мышь) + TouchSensor (long-press для тач-устройств, чтобы не блокировать прокрутку).
...
- **Тема**: class-based dark mode (`.dark` на `<html>`); выбор light/dark/system хранится в localStorage; статусы через CSS-переменные (`--status-sufficient/low/out`), в тёмной теме осветляются; акцентные кнопки в dark — `green-500`.
